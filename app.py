...
def fetch_espn_bpi() -> Tuple[pd.DataFrame, Optional[pd.DataFrame], str]:
    ...
    Returns (bpi_df, torvik_df_if_used, source_label)
      - source_label is "ESPN API", "ESPN HTML", or "Torvik fallback"
    """
...
def draw_card(row: pd.Series, brand_text: str = "CBB Edges", brand_logo_url: Optional[str] = None) -> Image.Image:
    ...
    d.text((left, top + 80), f"BPI {fmt_signed(bpi_a)}".replace("BPI —", "BPI —"), fill=(235, 240, 245), font=MED_FONT)

    d.text((left, top + 210), "@", fill=(200, 205, 220), font=MED_FONT)

    d.text((left, top + 300), home, fill=(255, 255, 255), font=TITLE_FONT)
    d.text((left, top + 380), f"BPI {fmt_signed(bpi_h)}".replace("BPI —", "BPI —"), fill=(235, 240, 245), font=MED_FONT)
...
if bpi_df is None or bpi_df.empty:
    st.error("Could not load any ratings (BPI or fallback). Try again later.")
    st.stop()
...
if bpi_source == "Torvik fallback":
    st.info("ESPN BPI was unavailable; showing Torvik ADJEM as a fallback for rating gaps.")
else:
